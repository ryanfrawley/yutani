...
//! (the [`crate::ansi`] parser feeds the dispatch loop). The central OSC
//! dispatcher, window-title (OSC 0/2) and hyperlink (OSC 8) handling, and the
//! semantic-mark storage live here; the feature-specific handlers split into
//! submodules:
//!
...
//!
//! Both submodules hold `impl Terminal` blocks. The shared types and pure
//! free functions stay in this file (the [`Terminal`] struct owns the state
//! fields) and the submodules reach them via `use super::*`; the private
//! handlers the dispatcher in this file calls are `pub(super)`.
